...
_Benign_race_end_
	...
	static void* HeapAllocateHelper(SizeT size, SizeT alignment, SizeT offset)
	{
		...
		HANDLE local_abi_heap = InterlockedCompareExchangePointer(&__abi_heap, nullptr, nullptr);
		...
		if (!local_abi_heap)
		{
			...
			local_abi_heap = InterlockedCompareExchangePointer(&__abi_heap, heap, nullptr);
		...
		case ::Platform::Details::HeapAllocationTrackingLevel::NoTracking:
			{
				LPVOID pMem = (alignment == 0)
					? HeapAlloc(local_abi_heap, 0, size)
