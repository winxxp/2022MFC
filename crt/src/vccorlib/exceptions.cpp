...
namespace Platform
{
	...
	namespace Details {
		...
		::Platform::Exception^ ReCreateException(long hrEx)
		{
			...
			if (hr != S_OK)
			{
// OriginateError in roerrorapi.h should be annotated with `_In_opt_ HSTRING message`
// as it calls RoOriginateError which is documented as accepting NULL successfully.
#pragma warning(suppress: 6387)
			...
			{
// TransformError in roerrorapi.h should be annotated with `_In_opt_ HSTRING message`
// as it calls RoTransformError which is documented as accepting NULL successfully.
#pragma warning(suppress: 6387)
